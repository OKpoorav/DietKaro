...
import {
  buildUnifiedClientSummary,
  DocSummaryInput,
} from "./ai/unified-summary";
...
export async function processReportDocument(reportId: string): Promise<void> {
  ...
  const mimeType =
    report.mimeType ?? (report.fileType === "pdf" ? "application/pdf" : null);
  ...
  try {
    ...
    const existingSummary = await prisma.reportSummary.findUnique({
      where: { reportId },
    });
    if (existingSummary?.summaryText) {
      ...
      logger.info("Report summary already exists, skipping AI extraction", {
        reportId,
      });
      return;
    }
    ...
    await prisma.clientReport.update({
      where: { id: reportId },
      data: { processingStatus: "summarizing" },
    });
    ...
    await prisma.clientDocumentSummary.deleteMany({
      ...
      if (client?.primaryDietitianId) {
        ...
        await notifSvc.sendNotification(
          ...
          `${client.fullName}'s ${report.reportType || "document"} has been processed`,
          {
            ...
            entityId: report.id,
            deepLink: `/dashboard/clients/${report.clientId}`,
          },
          "report_processed",
        );
      }
 ...
 */
export async function getCachedUnifiedSummary(
  clientId: string,
): Promise<string | null> {
  const existing = await prisma.clientDocumentSummary.findUnique({
    where: { clientId },
  });
 ...
 */
export async function buildAndCacheUnifiedSummary(
  clientId: string,
  _orgId: string,
): Promise<void> {
  ...
  const inputs: DocSummaryInput[] = summaries
    ...
    .map((s) => ({
      ...
      flags:
        ((s.extractedData as Record<string, unknown>)
          ?.dietary_flags as string[]) ?? [],
    }));

  if (inputs.length === 0) {
    logger.info("No report summaries found, skipping unified summary build", {
      clientId,
    });
    return;
  }
  ...
  logger.info("Unified summary built and cached", {
    clientId,
    docCount: inputs.length,
  });
}
