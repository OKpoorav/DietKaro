...
export function generateDietPlanPDF(plan: DietPlanWithRelations): PDFKit.PDFDocument {
    ...
    // ── General Guidelines — plan-level, shown under the date range ─────────
    if (plan.notesForClient) {
        const PADDING = 8;
        doc.fontSize(9);
        const textH = doc.heightOfString(plan.notesForClient, { width: PAGE_W - PADDING * 2, lineGap: 2 });
        const boxH = textH + PADDING * 2 + 12;
        const boxY = doc.y;
        doc.rect(40, boxY, PAGE_W, boxH).fill('#fef3c7');
        doc.rect(40, boxY, PAGE_W, boxH).strokeColor('#fcd34d').lineWidth(0.5).stroke();
        doc.fontSize(8).fillColor('#92400e')
            .text('GENERAL GUIDELINES', 40 + PADDING, boxY + PADDING - 2);
        doc.fontSize(9).fillColor('#78350f')
            .text(plan.notesForClient, 40 + PADDING, boxY + PADDING + 10, { width: PAGE_W - PADDING * 2, lineGap: 2 });
        doc.x = 40;
        doc.y = boxY + boxH + 8;
    }
...
<body>
    ...
    ${plan.notesForClient ? `<div class="notes"><h3>General Guidelines</h3><p>${escapeHtml(plan.notesForClient)}</p></div>` : ''}
    ${targetsHtml}
    <table>
