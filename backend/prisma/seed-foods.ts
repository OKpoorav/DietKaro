/**
 * Self-contained prepared foods seed (no src/ imports — safe to run in Docker runner).
 * Run: node_modules/.bin/tsx prisma/seed-foods.ts
 */
...
const COMMON_FOODS: FoodSeed[] = [
    // Breakfast
    { name: 'Idli', category: 'Breakfast', calories: 39, proteinG: 1.5, carbsG: 8.0, fatsG: 0.2, fiberG: 0.3, dietaryCategory: 'vegetarian', ingredientNames: ['White Rice (Raw)', 'Urad Dal (Black Gram Split)'] },
    { name: 'Dosa (Plain)', category: 'Breakfast', calories: 89, proteinG: 2.0, carbsG: 12.0, fatsG: 3.5, fiberG: 0.5, dietaryCategory: 'vegetarian', ingredientNames: ['White Rice (Raw)', 'Urad Dal (Black Gram Split)'] },
    { name: 'Masala Dosa', category: 'Breakfast', calories: 175, proteinG: 4.0, carbsG: 22.0, fatsG: 7.5, fiberG: 1.5, dietaryCategory: 'vegetarian', ingredientNames: ['White Rice (Raw)', 'Urad Dal (Black Gram Split)', 'Potato', 'Onion'] },
    ...
    { name: 'Uttapam', category: 'Breakfast', calories: 110, proteinG: 3.5, carbsG: 16.0, fatsG: 3.0, fiberG: 1.0, dietaryCategory: 'vegetarian', ingredientNames: ['White Rice (Raw)', 'Urad Dal (Black Gram Split)', 'Onion', 'Tomato'] },
    ...
    // Rice & Grains
    { name: 'White Rice (Cooked)', category: 'Grains', calories: 130, proteinG: 2.7, carbsG: 28.0, fatsG: 0.3, fiberG: 0.4, dietaryCategory: 'vegan', ingredientNames: ['White Rice (Raw)'] },
    { name: 'Brown Rice (Cooked)', category: 'Grains', calories: 111, proteinG: 2.6, carbsG: 23.0, fatsG: 0.9, fiberG: 1.8, dietaryCategory: 'vegan', ingredientNames: ['Brown Rice (Raw)'] },
    { name: 'Chapati / Roti (Wheat)', category: 'Grains', calories: 71, proteinG: 2.7, carbsG: 15.0, fatsG: 0.4, fiberG: 1.9, dietaryCategory: 'vegan', ingredientNames: ['Wheat Flour (Atta)'] },
    ...
    { name: 'Saag (Sarson da Saag)', category: 'Vegetables', calories: 95, proteinG: 4.0, carbsG: 10.0, fatsG: 5.0, fiberG: 4.5, dietaryCategory: 'vegetarian', ingredientNames: ['Spinach (Palak)', 'Fenugreek Leaves (Methi)', 'Ghee'] },
    ...
    { name: 'Tofu Bhurji', category: 'Vegetables', calories: 130, proteinG: 10.0, carbsG: 5.0, fatsG: 8.0, fiberG: 1.5, dietaryCategory: 'vegan', ingredientNames: ['Tofu (Firm)', 'Onion', 'Tomato'] },
    ...
    { name: 'Fish Curry', category: 'Non-Veg', calories: 135, proteinG: 18.0, carbsG: 3.0, fatsG: 5.5, dietaryCategory: 'non_veg', ingredientNames: ['Rohu Fish', 'Onion', 'Tomato'] },
    ...
    { name: 'Prawn Masala', category: 'Non-Veg', calories: 140, proteinG: 18.0, carbsG: 4.0, fatsG: 6.0, dietaryCategory: 'non_veg', ingredientNames: ['Prawns / Shrimp', 'Onion', 'Tomato'] },
    ...
    { name: 'Vada Pav', category: 'Snacks', calories: 290, proteinG: 5.0, carbsG: 35.0, fatsG: 14.0, dietaryCategory: 'vegetarian', ingredientNames: ['Potato', 'Besan (Chickpea Flour)'] },
    { name: 'Pav Bhaji', category: 'Snacks', calories: 320, proteinG: 6.0, carbsG: 40.0, fatsG: 15.0, fiberG: 4.0, dietaryCategory: 'vegetarian', ingredientNames: ['Potato', 'Cauliflower', 'Green Peas (Fresh)', 'Butter'] },
    ...
    { name: 'Lassi (Salted)', category: 'Dairy', calories: 60, proteinG: 3.0, carbsG: 6.0, fatsG: 2.0, dietaryCategory: 'vegetarian', ingredientNames: ['Curd / Yogurt'] },
    ...
    { name: 'Green Smoothie (Spinach + Banana)', category: 'Beverages', calories: 95, proteinG: 2.5, carbsG: 20.0, fatsG: 0.8, fiberG: 3.0, dietaryCategory: 'vegan', ingredientNames: ['Spinach (Palak)', 'Banana'] },
];

async function main() {
    console.log('🌱 Seeding prepared foods...\n');
    ...
    for (const food of COMMON_FOODS) {
        ...
        if (existing) {
            console.log(`⏭️  Skipped: ${food.name}`);
            skipped++;
            continue;
        }
        ...
        if (food.ingredientNames) {
            for (const ingName of food.ingredientNames) {
                const ing = await prisma.foodItem.findFirst({
                    where: { name: ingName, orgId: null },
                    select: { id: true },
                });
                if (ing) {
                    await prisma.foodItemIngredient.create({
                        data: { foodItemId: newFood.id, ingredientId: ing.id },
                    });
                    linked++;
                }
            }
    ...
    console.log(`\n📊 Done! Created: ${created}, Skipped: ${skipped}, Links: ${linked}`);
}
